...
PYBIND11_MODULE(_helios, m)
{
  ...
  asset.def_readwrite("id", &Asset::id).def_readwrite("name", &Asset::name);

  py::class_<AbstractBeamDeflector,
             AbstractBeamDeflectorWrap,
  ...
  abstract_beam_deflector
    .def(py::init<double, double, double>(),
         ...
         py::arg("scanFreqMax_Hz"),
         py::arg("scanFreqMin_Hz"))
  ...
  py::class_<OscillatingMirrorBeamDeflector,
             AbstractBeamDeflector,
             std::shared_ptr<OscillatingMirrorBeamDeflector>>
    oscillating_mirror_beam_deflector(m, "OscillatingMirrorBeamDeflector");
  oscillating_mirror_beam_deflector
    ...
    .def_readwrite("scan_product",
                   &OscillatingMirrorBeamDeflector::cfg_device_scanProduct)
    .def("clone", &OscillatingMirrorBeamDeflector::clone);

  py::class_<ConicBeamDeflector,
             AbstractBeamDeflector,
             std::shared_ptr<ConicBeamDeflector>>
    conic_beam_deflector(m, "ConicBeamDeflector");
  ...
  py::class_<FiberArrayBeamDeflector,
             AbstractBeamDeflector,
             std::shared_ptr<FiberArrayBeamDeflector>>
    fiber_array_beam_deflector(m, "FiberArrayBeamDeflector");
  fiber_array_beam_deflector
    .def(py::init<double, double, double, int>(),
         ...
         py::arg("scanFreqMin_Hz"),
         py::arg("numFibers"))
    .def_property("num_fibers",
                  &FiberArrayBeamDeflector::getNumFibers,
                  &FiberArrayBeamDeflector::setNumFibers)
    .def("clone", &FiberArrayBeamDeflector::clone);

  py::class_<PolygonMirrorBeamDeflector,
             AbstractBeamDeflector,
             std::shared_ptr<PolygonMirrorBeamDeflector>>
    polygon_mirror_beam_deflector(m, "PolygonMirrorBeamDeflector");
  polygon_mirror_beam_deflector
    .def(py::init<double, double, double, double>(),
         py::arg("scanAngleMax_rad"),
         py::arg("scanFreqMax_Hz"),
         py::arg("scanFreqMin_Hz"),
         py::arg("ScanAngleEffectiveMax_rad"))
    .def_property_readonly(
      "scan_angle_effective_max",
  ...
  aabb
    ...
    .def(py::init<glm::dvec3, glm::dvec3>(),
         "Construct AABB from min and max vertices")
    .def_property(
      "vertices",
      [](const AABB& aabb) {
      ...
      [](AABB& aabb, const std::vector<Vertex>& vertices) {
        if (vertices.size() != 2) {
          throw std::runtime_error(
            "Vertices array must have exactly 2 elements.");
        }
  ...
  abstract_detector
    .def(py::init<std::shared_ptr<Scanner>, double, double, double>(),
         py::arg("scanner"),
         py::arg("accuracy_m"),
         py::arg("rangeMin_m"),
  ...
  py::class_<FullWaveformPulseDetector,
             AbstractDetector,
             std::shared_ptr<FullWaveformPulseDetector>>
    full_waveform_pulse_detector(m, "FullWaveformPulseDetector");
  ...
  vertex.def(py::init<>())
    ...
    .def_property(
      "position",
      [](Vertex& self) { return self.pos; },
      [](Vertex& self, const glm::dvec3& position) { self.pos = position; })
  ...
  trajectory.def(py::init<>())
    ...
    .def_property(
      "position",
      [](Trajectory& self) { return self.position; },
      [](Trajectory& self, const glm::dvec3& position) {
        self.position = position;
      });

  py::class_<TrajectorySettings, std::shared_ptr<TrajectorySettings>>
    trajectory_settings(m, "TrajectorySettings");
  trajectory_settings.def(py::init<>())
  ...
  measurement.def(py::init<>())
    ...
    .def_property(
      ...
      [](Measurement& m, double gpsTime) {
        m.gpsTime = gpsTime * 1000000000.0;
      })
    .def_property(
      "position",
      [](Measurement& self) { return self.position; },
  ...
  py::class_<RandomNoiseSource<double>,
             NoiseSource<double>,
             RandomNoiseSourceWrap<double>>(m, "RandomNoiseSource")
    .def(py::init<>())

    .def("get_random_noise_type",
         &RandomNoiseSource<double>::getRandomNoiseType)
  ...
  py::class_<UniformNoiseSource<double>, RandomNoiseSource<double>>(
    m, "UniformNoiseSource")
    .def(py::init<double, double>(), py::arg("min") = 0.0, py::arg("max") = 1.0)

    .def_property("min",
                  &UniformNoiseSource<double>::getMin,
                  &UniformNoiseSource<double>::setMin)
    .def_property("max",
                  &UniformNoiseSource<double>::getMax,
                  &UniformNoiseSource<double>::setMax)

    .def("configure_uniform_noise",
         &UniformNoiseSource<double>::configureUniformNoise)
    .def("noise_function", &UniformNoiseSource<double>::noiseFunction)
    .def("get_random_noise_type",
         &UniformNoiseSource<double>::getRandomNoiseType)
  ...
  py::class_<ScannerHead, std::shared_ptr<ScannerHead>> scanner_head(
    m, "ScannerHead");
  scanner_head
  ...
  py::class_<EvalScannerHead, ScannerHead, std::shared_ptr<EvalScannerHead>>
    eval_scanner_head(m, "EvalScannerHead");
  eval_scanner_head.def(py::init<glm::dvec3, double>(),
                        py::arg("headRotationAxis"),
                        py::arg("headRotatePerSecMax_rad"));
  ...
  material.def(py::init<>())
    ...
    .def_property(
      ...
      [](Material& m, const std::vector<float>& value) {
        if (value.size() != 4) {
          throw std::runtime_error(
            "ambient_components must have exactly 4 elements");
        }
    ...
    .def_property(
      ...
      [](Material& m, const std::vector<float>& value) {
        if (value.size() != 4) {
          throw std::runtime_error(
            "diffuse_components must have exactly 4 elements");
        }
    ...
    .def_property(
      ...
      [](Material& m, const std::vector<float>& value) {
        if (value.size() != 4) {
          throw std::runtime_error(
            "specular_components must have exactly 4 elements");
        }
  ...
  platform_settings.def(py::init<>())
    ...
    .def_property("position",
                  ...
                  [](PlatformSettings& self, const glm::dvec3& pos) {
  ...
  scene.def(py::init<>())
    ...
    .def_property("default_reflectance",
                  &Scene::getDefaultReflectance,
  ...
  static_scene.def(py::init<>())
    ...
    .def("set_static_object_part",
         ...
         py::arg("id"),
         py::arg("part"))
    .def("append_static_object_part",
         &StaticScene::appendStaticObject,
         py::arg("part"))
    .def("remove_static_object_part",
  ...
  platform.def(py::init<>())
    ...
    .def_readwrite("device_relative_position",
                   &Platform::cfg_device_relativeMountPosition)
    .def_readwrite("device_relative_attitude",
    ...
    .def_readwrite("cached_end_target_angle_xy",
                   &Platform::cached_endTargetAngle_xy)
    .def_readwrite("cached_origin_to_target_angle_xy",
                   &Platform::cached_originToTargetAngle_xy)
    .def_readwrite("cached_target_to_next_angle_xy",
                   &Platform::cached_targetToNextAngle_xy)
    .def_readwrite("cached_distance_to_target_xy",
                   &Platform::cached_distanceToTarget_xy)

    .def_property("position", &Platform::getPosition, &Platform::setPosition)
    .def_property(
      "attitude",
      [](Platform& self) { return self.attitude; },
      [](Platform& self, const Rotation& attitude) {
        self.attitude = attitude;
      })
    .def_property(
      "absolute_mount_position",
      [](Platform& self) { return self.cached_absoluteMountPosition; },
      [](Platform& self, const glm::dvec3& position) {
        self.cached_absoluteMountPosition = position;
      })
    .def_property(
      "absolute_mount_attitude",
      [](Platform& self) { return self.cached_absoluteMountAttitude; },
      [](Platform& self, const Rotation& attitude) {
    ...
    .def_property(
      "cached_origin_to_target_dir_xy",
      [](const Platform& self) { return self.cached_originToTargetDir_xy; },
      [](Platform& self, const glm::dvec3& position) {
        self.cached_originToTargetDir_xy = position;
      })
    .def_property(
      "cached_target_to_next_dir_xy",
      [](const Platform& self) { return self.cached_targetToNextDir_xy; },
      [](Platform& self, const glm::dvec3& position) {
        self.cached_targetToNextDir_xy = position;
      });

  py::class_<MovingPlatform, Platform, std::shared_ptr<MovingPlatform>>
    moving_platform(m, "MovingPlatform");
  moving_platform

    .def(py::init<>())

    .def_property(
      "velocity", &MovingPlatform::getVelocity, &MovingPlatform::setVelocity)
  ...
  py::class_<SimplePhysicsPlatform,
             MovingPlatform,
             std::shared_ptr<SimplePhysicsPlatform>>
    simple_physics_platform(m, "SimplePhysicsPlatform");
  simple_physics_platform.def(py::init<>())
    ...
    .def("configure_step_magnitude",
         &SimplePhysicsPlatform::configureStepMagnitude)
    .def("check_speed_limit", &SimplePhysicsPlatform::checkSpeedLimit)
    .def("clone", &SimplePhysicsPlatform::clone);

  py::class_<GroundVehiclePlatform,
             SimplePhysicsPlatform,
             std::shared_ptr<GroundVehiclePlatform>>
    ground_vehicle_platform(m, "GroundVehiclePlatform");
  ground_vehicle_platform.def(py::init<>())
  ...
  py::class_<LinearPathPlatform,
             MovingPlatform,
             std::shared_ptr<LinearPathPlatform>>
    linear_path_platform(m, "LinearPathPlatform");
  linear_path_platform.def(py::init<>())
  ...
  py::class_<HelicopterPlatform,
             SimplePhysicsPlatform,
             std::shared_ptr<HelicopterPlatform>>
    helicopter_platform(m, "HelicopterPlatform");
  helicopter_platform.def(py::init<>())
    .def_readwrite("slowdown_distance_xy",
                   &HelicopterPlatform::cfg_slowdown_dist_xy)
    .def_readwrite("slowdown_magnitude",
                   &HelicopterPlatform::cfg_slowdown_magnitude)
    .def_readwrite("speedup_magnitude",
                   &HelicopterPlatform::cfg_speedup_magnitude)
    ...
    .def_readwrite("max_pitch_offset",
                   &HelicopterPlatform::cfg_max_pitch_offset)
    ...
    .def_readwrite("pitch_step_magnitude",
                   &HelicopterPlatform::cfg_pitchStepMagnitude)
    .def_readwrite("roll_step_magnitude",
                   &HelicopterPlatform::cfg_rollStepMagnitude)
    .def_readwrite("yaw_step_magnitude",
                   &HelicopterPlatform::cfg_yawStepMagnitude)
    .def_readwrite("alignment_threshold",
                   &HelicopterPlatform::cfg_alignmentThreshold)
    ...
    .def_readwrite("cache_turn_iterations",
                   &HelicopterPlatform::cache_turnIterations)
    ...
    .def_readwrite("cache_distance_threshold",
                   &HelicopterPlatform::cache_xyDistanceThreshold)
    .def_readwrite("cache_speedup_finished",
                   &HelicopterPlatform::cache_speedUpFinished)

    .def_property("heading_rad",
                  &HelicopterPlatform::getHeadingRad,
                  &HelicopterPlatform::setHeadingRad)
    ...
    .def("compute_alignment_angles",
         &HelicopterPlatform::computeAlignmentAngles)
    ...
    .def("compute_turn_distance_threshold",
         &HelicopterPlatform::computeTurnDistanceThreshold)
  ...
  swap_on_repeat_handler
    ...
    .def_property_readonly("num_target_replays",
                           &SwapOnRepeatHandler::getNumTargetReplays)
    .def_property("keep_crs",
                  &SwapOnRepeatHandler::isKeepCRS,
                  &SwapOnRepeatHandler::setKeepCRS)
    .def_property("discard_on_replay",
                  &SwapOnRepeatHandler::needsDiscardOnReplay,
                  &SwapOnRepeatHandler::setDiscardOnReplay)
  ...
  scanner
    ...
    .def_property(
      ...
      [](Scanner& self, const std::shared_ptr<helios::filems::FMSFacade>& fms) {
        self.fms = fms;
      })
    .def_property(
      "all_output_paths",
    ...
    .def_property(
      "pulse_freq_hz", &Scanner::getPulseFreq_Hz, &Scanner::setPulseFreq_Hz)
    ...
    .def_property(
      ...
      "A shared mutex for synchronized operations")
    .def_property(
      "write_waveform", &Scanner::isWriteWaveform, &Scanner::setWriteWaveform)

    .def_property(
      "write_pulse", &Scanner::isWritePulse, &Scanner::setWritePulse);
  ...
  helios_simulation.def(py::init<>())
    ...
    .def_property(
      "survey", &PyHeliosSimulation::getSurvey, &PyHeliosSimulation::setSurvey)
  ...
  py::class_<SingleScanner, Scanner, std::shared_ptr<SingleScanner>>
    single_scanner(m, "SingleScanner");
  single_scanner
    .def(py::init<double,
                  glm::dvec3,
                  Rotation,
    ...
    .def_property(
      ...
      [](SingleScanner& self) { return self.getSupportedPulseFreqs_Hz(0); },
      [](SingleScanner& self, std::list<int> pulse_freqs) {
        self.setSupportedPulseFreqs_Hz(pulse_freqs, 0);
      },
      py::return_value_policy::reference_internal)
    .def_property("pulse_freq_hz",
                  &SingleScanner::getPulseFreq_Hz,
                  &SingleScanner::setPulseFreq_Hz)

    .def(py::init<SingleScanner&>(), py::arg("scanner"))
    .def("apply_settings",
         &SingleScanner::applySettings,
         py::arg("settings"),
         py::arg("idx") = 0)
    .def("do_sim_step",
    ...
    .def("prepare_discretization",
         ...
         py::arg("idx") = 0)
    .def("calc_atmospheric_attenuation",
         py::overload_cast<size_t>(&SingleScanner::calcAtmosphericAttenuation,
                                   py::const_))
    .def("check_max_NOR",
         py::overload_cast<int, size_t>(&SingleScanner::checkMaxNOR))
    .def("calc_absolute_beam_attitude",
         py::overload_cast<size_t>(&SingleScanner::calcAbsoluteBeamAttitude))
    .def("get_specific_current_pulse_number",
         py::overload_cast<size_t>(&SingleScanner::getCurrentPulseNumber,
                                   py::const_))
    .def("get_specific_num_rays",
    ...
    .def("set_specific_pulse_length",
         py::overload_cast<double, size_t>(&SingleScanner::setPulseLength_ns))
    .def("get_specific_last_pulse_was_hit",
         py::overload_cast<size_t>(&SingleScanner::lastPulseWasHit, py::const_))
    .def("set_specific_last_pulse_was_hit",
         py::overload_cast<bool, size_t>(&SingleScanner::setLastPulseWasHit))
    .def(
      "get_specific_beam_divergence",
      py::overload_cast<size_t>(&SingleScanner::getBeamDivergence, py::const_))
    .def("set_specific_beam_divergence",
         py::overload_cast<double, size_t>(&SingleScanner::setBeamDivergence))

    .def("get_max_nor",
         py::overload_cast<size_t>(&SingleScanner::getMaxNOR, py::const_),
         py::arg("index"))
    .def("set_max_nor",
         py::overload_cast<int, size_t>(&SingleScanner::setMaxNOR),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_average_power",
         py::overload_cast<size_t>(&SingleScanner::getAveragePower, py::const_),
         py::arg("index"))
    .def("set_specific_average_power",
         py::overload_cast<double, size_t>(&SingleScanner::setAveragePower),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_beam_quality",
         py::overload_cast<size_t>(&SingleScanner::getBeamQuality, py::const_),
         py::arg("index"))
    .def("set_specific_beam_quality",
         py::overload_cast<double, size_t>(&SingleScanner::setBeamQuality),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_efficiency",
         py::overload_cast<size_t>(&SingleScanner::getEfficiency, py::const_),
         py::arg("index"))
    .def("set_specific_efficiency",
         py::overload_cast<double, size_t>(&SingleScanner::setEfficiency),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_receiver_diameter",
         py::overload_cast<size_t>(&SingleScanner::getReceiverDiameter,
                                   py::const_),
         py::arg("index"))
    .def("set_specific_receiver_diameter",
         py::overload_cast<double, size_t>(&SingleScanner::setReceiverDiameter),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_visibility",
         py::overload_cast<size_t>(&SingleScanner::getVisibility, py::const_),
         py::arg("index"))
    .def("set_specific_visibility",
         py::overload_cast<double, size_t>(&SingleScanner::setVisibility),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_wavelength",
         py::overload_cast<size_t>(&SingleScanner::getWavelength, py::const_),
         py::arg("index"))
    .def("set_specific_wavelength",
         py::overload_cast<double, size_t>(&SingleScanner::setWavelength),
         py::arg("value"),
         py::arg("index"))

    .def("get_specific_atmospheric_extinction",
         py::overload_cast<size_t>(&SingleScanner::getAtmosphericExtinction,
                                   py::const_),
         py::arg("index"))
    .def("set_specific_atmospheric_extinction",
         py::overload_cast<double, size_t>(
           &SingleScanner::setAtmosphericExtinction),
         py::arg("value"),
         py::arg("index"))

    .def(
      "get_specific_beam_waist_radius",
      py::overload_cast<size_t>(&SingleScanner::getBeamWaistRadius, py::const_),
      py::arg("index"))
    .def("set_specific_beam_waist_radius",
         py::overload_cast<double, size_t>(&SingleScanner::setBeamWaistRadius),
         py::arg("value"),
         py::arg("index"))

    .def("get_head_relative_emitter_attitude",
         py::overload_cast<size_t>(
           &SingleScanner::getHeadRelativeEmitterAttitude, py::const_),
         py::arg("index"))
    .def("set_head_relative_emitter_attitude",
         py::overload_cast<const Rotation&, size_t>(
           &SingleScanner::setHeadRelativeEmitterAttitude),
         py::arg("value"),
         py::arg("index"))
    ...
    .def(
      ...
      py::arg("time_wave"),
      py::arg("index"))
    .def("get_peak_intensity_index",
         &SingleScanner::getPeakIntensityIndex,
         py::arg("index"))
    .def("set_peak_intensity_index",
         &SingleScanner::setPeakIntensityIndex,
         py::arg("value"),
         py::arg("index"))

    .def("get_scanner_head", &SingleScanner::getScannerHead, py::arg("index"))
    .def("set_scanner_head",
         &SingleScanner::setScannerHead,
         py::arg("scanner_head"),
         py::arg("index"))

    .def(
      "get_beam_deflector", &SingleScanner::getBeamDeflector, py::arg("index"))
    .def("set_beam_deflector",
         &SingleScanner::setBeamDeflector,
         py::arg("beam_deflector"),
         py::arg("index"))

    .def("get_max_NOR", &SingleScanner::getMaxNOR, py::arg("index"))
    .def("set_max_NOR",
         &SingleScanner::setMaxNOR,
         py::arg("value"),
         py::arg("index"))
    .def("clone", &SingleScanner::clone);

  py::class_<MultiScanner, Scanner, std::shared_ptr<MultiScanner>>
    multi_scanner(m, "MultiScanner");
  multi_scanner
    .def(py::init<std::vector<ScanningDevice>,
                  std::string,
                  const std::list<int>&,
                  bool,
                  bool,
    ...
    .def("prepare_discretization",
         ...
         py::arg("idx"))
    .def("calc_absolute_beam_attitude",
         &MultiScanner::calcAbsoluteBeamAttitude,
         py::arg("idx"))
    .def("calc_atmospheric_attenuation",
         &MultiScanner::calcAtmosphericAttenuation,
         py::arg("idx"))
    .def("check_max_nor",
         &MultiScanner::checkMaxNOR,
         py::arg("nor"),
         py::arg("idx"))
    .def("compute_subrays", &MultiScanner::computeSubrays)
    .def("initialize_full_waveform",
         &MultiScanner::initializeFullWaveform,
         py::arg("min_hit_dist_m"),
         py::arg("max_hit_dist_m"),
         py::arg("min_hit_time_ns"),
         py::arg("max_hit_time_ns"),
         py::arg("ns_per_bin"),
    ...
    .def("set_device_index",
         &MultiScanner::setDeviceIndex,
         py::arg("new_idx"),
         py::arg("old_idx"))

    .def("get_device_id", &MultiScanner::getDeviceId, py::arg("idx"))
    .def("set_device_id",
         &MultiScanner::setDeviceId,
         py::arg("device_id"),
         py::arg("idx"))

    .def("get_pulse_length", &MultiScanner::getPulseLength_ns, py::arg("idx"))
    .def("set_pulse_length",
         &MultiScanner::setPulseLength_ns,
         py::arg("pulse_length"),
         py::arg("idx"))

    .def(
      "get_beam_divergence", &MultiScanner::getBeamDivergence, py::arg("idx"))
    .def("set_beam_divergence",
         &MultiScanner::setBeamDivergence,
         py::arg("beam_divergence"),
         py::arg("idx"))

    .def("get_average_power", &MultiScanner::getAveragePower, py::arg("idx"))
    .def("set_average_power",
         &MultiScanner::setAveragePower,
         py::arg("average_power"),
         py::arg("idx"))

    .def("get_num_rays", &MultiScanner::getNumRays, py::arg("idx"))
    .def("set_num_rays",
         &MultiScanner::setNumRays,
         py::arg("num_rays"),
         py::arg("idx"))

    .def(
      "get_last_pulse_was_hit", &MultiScanner::lastPulseWasHit, py::arg("idx"))
    .def("set_last_pulse_was_hit",
         &MultiScanner::setLastPulseWasHit,
         py::arg("last_pulse_was_hit"),
         py::arg("idx"))

    .def("get_scanner_head", &MultiScanner::getScannerHead, py::arg("idx"))
    .def("set_scanner_head",
         &MultiScanner::setScannerHead,
         py::arg("scanner_head"),
         py::arg("idx"))

    .def("get_beam_deflector", &MultiScanner::getBeamDeflector, py::arg("idx"))
    .def("set_beam_deflector",
         &MultiScanner::setBeamDeflector,
         py::arg("beam_deflector"),
         py::arg("idx"))

    .def("get_detector", &MultiScanner::getDetector, py::arg("idx"))
    .def("set_detector",
         &MultiScanner::setDetector,
         py::arg("detector"),
         py::arg("idx"))

    .def("get_fwf_settings", &MultiScanner::getFWFSettings, py::arg("idx"))
    .def("set_fwf_settings",
         &MultiScanner::setFWFSettings,
         py::arg("fwf_settings"),
         py::arg("idx"))

    .def("get_beam_quality", &MultiScanner::getBeamQuality, py::arg("idx"))
    .def("set_beam_quality",
         &MultiScanner::setBeamQuality,
         py::arg("beam_quality"),
         py::arg("idx"))

    .def("get_efficiency", &MultiScanner::getEfficiency, py::arg("idx"))
    .def("set_efficiency",
         &MultiScanner::setEfficiency,
         py::arg("efficiency"),
         py::arg("idx"))

    .def("get_receiver_diameter",
         &MultiScanner::getReceiverDiameter,
         py::arg("idx"))
    .def("set_receiver_diameter",
         &MultiScanner::setReceiverDiameter,
         py::arg("receiver_diameter"),
         py::arg("idx"))

    .def("get_visibility", &MultiScanner::getVisibility, py::arg("idx"))
    .def("set_visibility",
         &MultiScanner::setVisibility,
         py::arg("visibility"),
         py::arg("idx"))

    .def("get_wavelength", &MultiScanner::getWavelength, py::arg("idx"))
    .def("set_wavelength",
         &MultiScanner::setWavelength,
         py::arg("wavelength"),
         py::arg("idx"))

    .def("get_atmospheric_extinction",
         &MultiScanner::getAtmosphericExtinction,
         py::arg("idx"))
    .def("set_atmospheric_extinction",
         &MultiScanner::setAtmosphericExtinction,
         py::arg("atmospheric_extinction"),
         py::arg("idx"))

    .def("get_beam_waist_radius",
         &MultiScanner::getBeamWaistRadius,
         py::arg("idx"))
    .def("set_beam_waist_radius",
         &MultiScanner::setBeamWaistRadius,
         py::arg("beam_waist_radius"),
         py::arg("idx"))

    .def("get_head_relative_emitter_position",
         &MultiScanner::getHeadRelativeEmitterPosition,
         py::arg("idx"))
    .def("set_head_relative_emitter_position",
         &MultiScanner::setHeadRelativeEmitterPosition,
         py::arg("pos"),
         py::arg("idx"))

    .def("get_head_relative_emitter_attitude",
         &MultiScanner::getHeadRelativeEmitterAttitude,
         py::arg("idx"))
    .def("set_head_relative_emitter_attitude",
         &MultiScanner::setHeadRelativeEmitterAttitude,
         py::arg("attitude"),
         py::arg("idx"))

    .def("get_head_relative_emitter_position_by_ref",
         &MultiScanner::getHeadRelativeEmitterPositionByRef,
         py::arg("idx") = 0)
    .def("get_head_relative_emitter_attitude_by_ref",
         &MultiScanner::getHeadRelativeEmitterAttitudeByRef,
         py::arg("idx") = 0)

    .def("get_time_wave", &MultiScanner::getTimeWave, py::arg("index"))
    .def("set_time_wave",
         static_cast<void (MultiScanner::*)(std::vector<double>&, size_t)>(
           &MultiScanner::setTimeWave),
         py::arg("time_wave"),
         py::arg("index"))
    ...
    .def("set_num_time_bins",
         &MultiScanner::setNumTimeBins,
         py::arg("num_time_bins"),
         py::arg("idx"))

    .def("get_peak_intensity_index",
         &MultiScanner::getPeakIntensityIndex,
         py::arg("idx"))
    .def("set_peak_intensity_index",
         &MultiScanner::setPeakIntensityIndex,
         py::arg("peak_intensity_index"),
         py::arg("idx"))

    .def("get_received_energy_min",
         &MultiScanner::getReceivedEnergyMin,
         py::arg("idx"))
    .def("set_received_energy_min",
         &MultiScanner::setReceivedEnergyMin,
         py::arg("received_energy_min"),
         py::arg("idx"))
    .def("clone", &MultiScanner::clone);

  py::class_<Simulation, SimulationWrap, std::shared_ptr<Simulation>>
  ...
  simulation
    ...
    .def_property("sim_speed_factor",
                  &Simulation::getSimSpeedFactor,
                  &Simulation::setSimSpeedFactor)
  ...
  py::class_<helios::filems::FMSFacade,
             std::shared_ptr<helios::filems::FMSFacade>>
    fms_facade(m, "FMSFacade");
  fms_facade
    .def(py::init<>())
  ...
  py::class_<helios::filems::FMSWriteFacade,
             std::shared_ptr<helios::filems::FMSWriteFacade>>
    fms_write_facade(m, "FMSWriteFacade");
  fms_write_facade.def(py::init<>())
    .def("disconnect", &helios::filems::FMSWriteFacade::disconnect)
    .def("get_measurement_writer_output_path",
         [](helios::filems::FMSWriteFacade& self) {
           return self.getMeasurementWriterOutputPath().string();
         });

  py::class_<helios::filems::FMSFacadeFactory,
             std::shared_ptr<helios::filems::FMSFacadeFactory>>
    fms_facade_factory(m, "FMSFacadeFactory");
  fms_facade_factory
    .def(py::init<>())
    ...
    .def("build_facade",
         py::overload_cast<const std::string&,
                           ...
                           bool,
                           Survey&,
                           bool>(&FMSFacadeFactory::buildFacade),
         py::arg("outdir"),
         py::arg("lasScale"),
         py::arg("lasOutput"),
         py::arg("las10"),
  ...
  py::class_<SurveyPlayback, Simulation, std::shared_ptr<SurveyPlayback>>
    survey_playback(m, "SurveyPlayback");
  survey_playback
    .def(py::init<std::shared_ptr<Survey>,
                  int,
                  std::shared_ptr<PulseThreadPoolInterface>,
                  int,
                  std::string,
  ...
  py::class_<PulseThreadPoolFactory, std::shared_ptr<PulseThreadPoolFactory>>(
    m, "PulseThreadPoolFactory")
    ...
    .def("make_basic_pulse_thread_pool",
         &PulseThreadPoolFactory::makeBasicPulseThreadPool)
    .def("make_pulse_warehouse_thread_pool",
         &PulseThreadPoolFactory::makePulseWarehouseThreadPool);

  py::class_<PulseThreadPoolInterface,
             PulseThreadPoolInterfaceWrap,
             std::shared_ptr<PulseThreadPoolInterface>>(
    m, "PulseThreadPoolInterface")
    .def(py::init<>())
  ...
  py::class_<PulseWarehouseThreadPool,
             PulseThreadPoolInterface,
             std::shared_ptr<PulseWarehouseThreadPool>>(
    m, "PulseWarehouseThreadPool")
    .def(py::init<std::size_t, double, std::size_t>(),
         py::arg("_pool_size"),
         py::arg("deviceAccuracy"),
         py::arg("maxTasks") = 256)
    .def("run_pulse_task",
         &PulseWarehouseThreadPool::run_pulse_task,
         py::arg("dropper"))
    .def("try_run_pulse_task",
         &PulseWarehouseThreadPool::try_run_pulse_task,
         py::arg("dropper"))
    .def("join", &PulseWarehouseThreadPool::join);

  py::class_<TaskDropper<PulseTask,
                         PulseThreadPoolInterface,
    ...
    .def("add",
         (bool (TaskDropper<PulseTask,
                            ...
                            NoiseSource<double>&>::*)(
           std::shared_ptr<
             PulseTask>))&TaskDropper<PulseTask,
                                      PulseThreadPoolInterface,
                                      std::vector<std::vector<double>>&,
                                      RandomnessGenerator<double>&,
                                      RandomnessGenerator<double>&,
    ...
    .def("drop",
         (void (TaskDropper<PulseTask,
           ...
           RandomnessGenerator<double>&,
           RandomnessGenerator<double>&,
           NoiseSource<double>&))&TaskDropper<PulseTask,
                                              PulseThreadPoolInterface,
                                              std::vector<std::vector<double>>&,
                                              RandomnessGenerator<double>&,
                                              RandomnessGenerator<double>&,
  ...
  py::class_<ScanningPulseProcess,
    ...
    .def("get_all_measurements",
         &ScanningPulseProcess::getAllMeasurements,
         py::return_value_policy::reference_internal)
  ...
  py::class_<WarehouseScanningPulseProcess,
             ScanningPulseProcess,
  ...
  kdgrove.def(py::init<size_t>(), py::arg("initTreesCapacity") = 1);

  py::class_<KDGroveFactory, std::shared_ptr<KDGroveFactory>>(m,
                                                              "KDGroveFactory")
    .def(py::init<std::shared_ptr<KDTreeFactory>>(), py::arg("kdtf"));

  py::class_<KDTreeFactory, KDTreeFactoryWrap, std::shared_ptr<KDTreeFactory>>(
    m, "KDTreeFactory")
    .def(py::init<>());

  py::class_<SimpleKDTreeFactory,
             KDTreeFactory,
             std::shared_ptr<SimpleKDTreeFactory>>(m, "SimpleKDTreeFactory")
    .def(py::init<>());

  py::class_<SimpleKDTreeGeometricStrategy,
             std::shared_ptr<SimpleKDTreeGeometricStrategy>>(
    m, "SimpleKDTreeGeometricStrategy")
    .def(py::init<SimpleKDTreeFactory&>(), py::arg("kdtf"));

  py::class_<MultiThreadKDTreeFactory,
             SimpleKDTreeFactory,
             std::shared_ptr<MultiThreadKDTreeFactory>>(
    m, "MultiThreadKDTreeFactory")
    .def(py::init<std::shared_ptr<SimpleKDTreeFactory>,
                  std::shared_ptr<SimpleKDTreeGeometricStrategy>,
                  size_t,
                  size_t>(),
         py::arg("kdtf"),
  ...
  py::class_<SAHKDTreeGeometricStrategy,
             SimpleKDTreeGeometricStrategy,
             std::shared_ptr<SAHKDTreeGeometricStrategy>>(
    m, "SAHKDTreeGeometricStrategy")
    .def(py::init<SAHKDTreeFactory&>(), py::arg("kdtf"));

  py::class_<SAHKDTreeFactory,
             SimpleKDTreeFactory,
             std::shared_ptr<SAHKDTreeFactory>>(m, "SAHKDTreeFactory")
  ...
  py::class_<MultiThreadSAHKDTreeFactory,
             MultiThreadKDTreeFactory,
             std::shared_ptr<MultiThreadSAHKDTreeFactory>>(
    m, "MultiThreadSAHKDTreeFactory")
    .def(py::init<std::shared_ptr<SimpleKDTreeFactory>,
                  std::shared_ptr<SimpleKDTreeGeometricStrategy>,
                  size_t,
                  size_t>(),
  ...
  py::class_<AxisSAHKDTreeFactory,
             SAHKDTreeFactory,
             std::shared_ptr<AxisSAHKDTreeFactory>>(m, "AxisSAHKDTreeFactory")
    .def(py::init<size_t, double, double, double>(),
         py::arg("lossNodes") = 21,
         py::arg("ci") = 1,
         py::arg("cl") = 1,
  ...
  py::class_<AxisSAHKDTreeGeometricStrategy,
             SAHKDTreeGeometricStrategy,
             std::shared_ptr<AxisSAHKDTreeGeometricStrategy>>(
    m, "AxisSAHKDTreeGeometricStrategy")
    .def(py::init<AxisSAHKDTreeFactory&>(), py::arg("kdtf"));

  py::class_<FastSAHKDTreeFactory,
             SAHKDTreeFactory,
             std::shared_ptr<FastSAHKDTreeFactory>>(m, "FastSAHKDTreeFactory")
    .def(py::init<size_t, double, double, double>(),
         py::arg("lossNodes") = 32,
         py::arg("ci") = 1,
         py::arg("cl") = 1,
  ...
  py::class_<FastSAHKDTreeGeometricStrategy,
             SAHKDTreeGeometricStrategy,
