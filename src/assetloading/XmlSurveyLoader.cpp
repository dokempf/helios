                                     ...
                                     bool rebuildScene)
{
  ...
  // Load legs
  loadLegs(surveyNode->FirstChildElement("leg"),
...
void
XmlSurveyLoader::handleCoreOverloading(tinyxml2::XMLElement* surveyNode,
                                       std::shared_ptr<Survey> survey)
{
...
void
XmlSurveyLoader::loadLegs(tinyxml2::XMLElement* legNodes,
                          ...
                          std::vector<std::shared_ptr<Leg>>& legs)
{
  ...
  while (legNodes != nullptr) {
    ...
    leg->mScannerSettings = scannerSettings->cherryPick(
      leg->mScannerSettings, scannerFields, &templateFields);
...
void
XmlSurveyLoader::applySceneShift(tinyxml2::XMLElement* surveyNode,
...
void
XmlSurveyLoader::configureDefaultRandomnessGenerator(
...
void
XmlSurveyLoader::loadPlatformNoise(tinyxml2::XMLElement* surveyNode,
                                   std::shared_ptr<Platform> platform)
{
  // Position noise
...
void
XmlSurveyLoader::integrateSurveyAndLegs(std::shared_ptr<Survey> survey)
{
  // Obtain legs
