                                      ...
                                      std::string const separator)
{
  ...
  // Prepare for parsing
  bool minCornerXFound = false, minCornerYFound = false,
       minCornerZFound = false, maxCornerXFound = false,
       maxCornerYFound = false, maxCornerZFound = false, splitXFound = false,
       splitYFound = false, splitZFound = false;
  double minCornerX, minCornerY, minCornerZ, maxCornerX, maxCornerY, maxCornerZ;
  ...
  validateDetailed(minCornerXFound,
                   minCornerYFound,
                   minCornerZFound,
                   maxCornerXFound,
                   maxCornerYFound,
  ...
  char format1[4096];
  snprintf(
    format1, sizeof(format1), "%s%s%s%s%s", "%zu", sep, "%zu", sep, "%zu");
  char format2[4096];
  snprintf(format2,
           sizeof(format2),
...
void
VoxelFileParser::cleanLines(std::vector<std::string>& lines,
                            ...
                            double& maxPad)
{
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    if (isBlankLine(lines[i]))
      remove = true;
    else if (handleSpecLine(lines[i],
                            minCornerXFound,
                            minCornerX,
                            minCornerYFound,
                            minCornerY,
...
bool
VoxelFileParser::isBlankLine(std::string const& line)
{
...
bool
VoxelFileParser::handleSpecLine(std::string const& line,
                                bool& minCornerXFound,
                                double& minCornerX,
                                bool& minCornerYFound,
                                double& minCornerY,
...
void
VoxelFileParser::handleSpec(std::string const& line,
                            bool& minCornerXFound,
                            double& minCornerX,
                            bool& minCornerYFound,
                            double& minCornerY,
                            ...
                            double& maxPad)
{
  ...
  std::string fieldName =
    line.substr(fieldNameStart, fieldNameEnd - fieldNameStart);
  std::string fieldValue = line.substr(fieldNameEnd + 1);
  int n;

  if (fieldName == "min_corner") {
    n = sscanf(
      fieldValue.c_str(), "%lf %lf %lf", &minCornerX, &minCornerY, &minCornerZ);
    if (n >= 1)
      minCornerXFound = true;
    if (n >= 2)
      minCornerYFound = true;
    if (n >= 3)
      minCornerZFound = true;
  } else if (fieldName == "max_corner") {
    n = sscanf(
      fieldValue.c_str(), "%lf %lf %lf", &maxCornerX, &maxCornerY, &maxCornerZ);
    if (n >= 1)
      maxCornerXFound = true;
    if (n >= 2)
      maxCornerYFound = true;
    if (n >= 3)
      maxCornerZFound = true;
  } else if (fieldName == "split") {
    n = sscanf(fieldValue.c_str(), "%zu %zu %zu", &splitX, &splitY, &splitZ);
    if (n >= 1)
      splitXFound = true;
    if (n >= 2)
      splitYFound = true;
    ...
    while (fieldName != "max_pad" && i < line.size()) {
      ...
      fieldName = line.substr(fieldNameStart, fieldNameEnd - fieldNameStart);
...
void
VoxelFileParser::cleanLine(std::string& line)
{
  ...
  line.erase(backwardEraseStart - backwardEraseLen - forwardEraseLen + 1,
             backwardEraseLen);
}

void
VoxelFileParser::removeHeaderLines(std::vector<std::string>& lines,
                                   size_t numHeaderLines)
{
...
void
VoxelFileParser::validateDetailed(bool minCornerXFound,
                                  bool minCornerYFound,
                                  bool minCornerZFound,
                                  bool maxCornerXFound,
                                  bool maxCornerYFound,
                                        ...
                                        double maxPad)
{
  ...
  values.push_back(bsInterc);
  values.push_back(bsPoten);
  values.push_back(gDist);
  values.push_back(lMeanTotal);
  values.push_back(lgTotal);
  values.push_back(transmit);
  values.push_back(atten);
