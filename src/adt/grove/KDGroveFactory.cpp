                                   ...
                                   std::vector<int>& treePrimitives)
{
  ...
  for (std::size_t i = 0; i < numTrees; ++i) {
    ...
    if (kdgrove->getSubjects()[i] != nullptr)
      ++(stats.numDynTrees);
    else
      ++(stats.numStaticTrees);
    ...
    if (i == 0) { // Init value for min/max
      ...
      stats.minBuildingTime =
        std::min<double>(stats.minBuildingTime, buildingTimes[i]);
      stats.maxBuildingTime =
        std::max<double>(stats.maxBuildingTime, buildingTimes[i]);
      stats.minTreePrimitives =
        std::min<int>(stats.minTreePrimitives, treePrimitives[i]);
      stats.maxTreePrimitives =
        std::max<int>(stats.maxTreePrimitives, treePrimitives[i]);
      stats.minMaxPrimsInLeaf =
        std::min<int>(stats.minMaxPrimsInLeaf, root->stats_maxNumPrimsInLeaf);
      stats.maxMaxPrimsInLeaf =
        std::max<int>(stats.maxMaxPrimsInLeaf, root->stats_maxNumPrimsInLeaf);
      stats.minMinPrimsInLeaf =
        std::min<int>(stats.minMinPrimsInLeaf, root->stats_minNumPrimsInLeaf);
      stats.maxMinPrimsInLeaf =
        std::max<int>(stats.maxMinPrimsInLeaf, root->stats_minNumPrimsInLeaf);
      stats.minMaxDepth =
        std::min<int>(stats.minMaxDepth, root->stats_maxDepthReached);
      stats.maxMaxDepth =
        std::max<int>(stats.maxMaxDepth, root->stats_maxDepthReached);
      stats.minSurfaceArea =
        std::min<double>(stats.minSurfaceArea, root->surfaceArea);
      stats.maxSurfaceArea =
        std::max<double>(stats.maxSurfaceArea, root->surfaceArea);
      stats.minNumInterior =
        std::min<int>(stats.minNumInterior, root->stats_numInterior);
      stats.maxNumInterior =
        std::max<int>(stats.maxNumInterior, root->stats_numInterior);
      stats.minNumLeaves =
        std::min<int>(stats.minNumLeaves, root->stats_numLeaves);
      stats.maxNumLeaves =
        std::max<int>(stats.maxNumLeaves, root->stats_numLeaves);
      stats.minCost = std::min<double>(stats.minCost, root->stats_totalCost);
      stats.maxCost = std::max<double>(stats.maxCost, root->stats_totalCost);
    }
  ...
  for (size_t i = 0; i < numTrees; ++i) {
    KDTreeNodeRoot* root =
      (KDTreeNodeRoot*)kdgrove->getTreePointer(i)->root.get();
    stats.stdevBuildingTime +=
      std::pow(buildingTimes[i] - stats.meanBuildingTime, 2);
    stats.stdevTreePrimitives +=
      std::pow(treePrimitives[i] - stats.meanTreePrimitives, 2);
    stats.stdevMaxPrimsInLeaf =
      std::pow(root->stats_maxNumPrimsInLeaf - stats.meanMaxPrimsInLeaf, 2);
    stats.stdevMinPrimsInLeaf =
      std::pow(root->stats_minNumPrimsInLeaf - stats.meanMinPrimsInLeaf, 2);
    stats.stdevMaxDepth =
      std::pow(root->stats_maxDepthReached - stats.meanMaxDepth, 2);
    stats.stdevSurfaceArea =
      std::pow(root->surfaceArea - stats.meanSurfaceArea, 2);
    stats.stdevNumInterior =
      std::pow(root->stats_numInterior - stats.meanNumInterior, 2);
    stats.stdevNumLeaves =
      std::pow(root->stats_numLeaves - stats.meanNumLeaves, 2);
    stats.stdevCost = std::pow(root->stats_totalCost - stats.meanCost, 2);
  }
...
KDGroveFactory::makeCommon(std::vector<std::shared_ptr<ScenePart>> parts,
                           ...
                           bool const reportKDTreeStats)
{
  auto collectPartRefs = [](std::shared_ptr<ScenePart> const& part) {
    std::vector<GeometryRef> out;
    if (part == nullptr) {
      return out;
    }
    out.reserve(part->geometryCount());
    for (std::size_t i = 0; i < part->geometryCount(); ++i) {
      GeometryRef ref{ part, i };
      if (ref.isValid()) {
        out.push_back(ref);
      }
    }
  ...
  for (std::shared_ptr<ScenePart>& part : parts) {
    ...
    TimeWatcher tw;
    tw.start();
    std::vector<GeometryRef> partRefs = collectPartRefs(part);
    std::shared_ptr<KDTreeNodeRoot> kdtree = std::shared_ptr<KDTreeNodeRoot>(
      safe ? kdtf->makeFromGeometryRefs(
               partRefs, computeKDTreeStats, reportKDTreeStats)
           : kdtf->makeFromGeometryRefsUnsafe(
               partRefs, computeKDTreeStats, reportKDTreeStats));
    ...
    numPrimitives.push_back(partRefs.size());
  }
  ...
  bool const reportKDTreeStats)
{
  ...
  auto buildTree = [&](std::vector<GeometryRef>& refs,
                       std::shared_ptr<ScenePart> const& part) {
    TimeWatcher tw;
    tw.start();
    std::shared_ptr<KDTreeNodeRoot> kdtree = std::shared_ptr<KDTreeNodeRoot>(
      safe ? kdtf->makeFromGeometryRefs(
               refs, computeKDTreeStats, reportKDTreeStats)
           : kdtf->makeFromGeometryRefsUnsafe(
               refs, computeKDTreeStats, reportKDTreeStats));
    KDGroveSubject* subject = nullptr;
    if (part != nullptr &&
        part->getType() == ScenePart::ObjectType::DYN_MOVING_OBJECT) {
      subject = (DynMovingObject*)part.get();
      subject->registerObserverGrove(kdgrove);
    }
    kdgrove->addSubject(
    ...
    tw.stop();
    buildingTimes.push_back(tw.getElapsedDecimalSeconds());
    numPrimitives.push_back(refs.size());
  };

  std::vector<GeometryRef> mergedRefs;
  for (std::shared_ptr<ScenePart> const& part : _parts) {
    if (part == nullptr) {
      continue;
    }
    std::vector<GeometryRef> partRefs = collectPartRefs(part);
    if (part->getType() == ScenePart::ObjectType::DYN_MOVING_OBJECT) {
      buildTree(partRefs, part);
    } else {
      mergedRefs.insert(mergedRefs.end(), partRefs.begin(), partRefs.end());
    }
  }
  if (!mergedRefs.empty()) {
    buildTree(mergedRefs, nullptr);
  }
