...
// ************************* //
void
FullWaveformPulseRunnable::operator()(
...
)
{
  ...
  if (scanner->isWritePulse())
    capturePulse(beamDir);

  // NOTE:
  // With beam divergence / full waveform being simulated, this is not perfect,
  // since a sub-ray might hit the scene even if the central ray does now.
  // However, this check is a very important performance optimization, so we
  // should keep it nevertheless. sbecht 2016-04-24
...
// ************************** //
void
FullWaveformPulseRunnable::initialize()
{
  AbstractPulseRunnable::initialize();
  fwDetector = std::static_pointer_cast<FullWaveformPulseDetector>(detector);
}
void
...
)
{
  ...
  glm::dvec3 subrayDirection =
    pulse.getAttitude().applyTo(subrayRotation).applyTo(Directions::forward);
  vector<double> tMinMax =
    scene.getAABB()->getRayIntersection(pulse.getOriginRef(), subrayDirection);
#if DATA_ANALYTICS >= 2
  ...
  while (rayContinues) {
    ...
    if (intersect != nullptr && intersect->prim.isValid()) {
#if DATA_ANALYTICS >= 2
      ...
      if (!scanner->isFixedIncidenceAngle()) {
        incidenceAngle =
          PrimitiveAccessor::getIncidenceAngle_rad(intersect->prim,
                                                   pulse.getOriginRef(),
                                                   subrayDirection,
                                                   intersect->point);
      }
      ...
      double distance = glm::distance(intersect->point, pulse.getOriginRef());

      // Distance must be inside [rangeMin, rangeMax] interval
      if (detector->isDistanceNotInRange(distance))
        continue;

      // Distance between beam's center line and intersection point:
      double intensity = 0.0;
      if (PrimitiveAccessor::canComputeSigmaWithLadLut(intersect->prim)) {
        // LadLut based intensity computation
        double const sigma = PrimitiveAccessor::computeSigmaWithLadLut(
          intersect->prim, subrayDirection);
        ...
        std::shared_ptr<Material> mat =
          PrimitiveAccessor::getMaterial(intersect->prim);
        if (mat == nullptr) {
          continue;
        }
      ...
      if (PrimitiveAccessor::canHandleIntersections(intersect->prim)) {
        AABB aabb = PrimitiveAccessor::getAABB(intersect->prim);
        glm::dvec3 outsideIntersectionPoint =
          RayUtils::obtainPointAfterTraversing(
            aabb, subrayOrigin, subrayDirection, 0.0);
        IntersectionHandlingResult ihr =
          PrimitiveAccessor::onRayIntersection(intersect->prim,
                                               intersectionHandlingNoiseSource,
                                               subrayDirection,
                                               intersect->point,
        ...
        if (ihr.canRayContinue()) { // Subray can continue
          ...
          tMinMax =
            scene.getAABB()->getRayIntersection(subrayOrigin, subrayDirection);
          rayContinues = true;
        } else { // Update distance considering noise
          distance =
            glm::distance(ihr.getIntersectionPoint(), pulse.getOriginRef());
        }
      }
...
#if DATA_ANALYTICS >= 2
  if (subrayHit)
    HDA_GV.incrementIntersectiveSubraysCount();
  else
    HDA_GV.incrementNonIntersectiveSubraysCount();
#endif
}
...
)
{
  ...
  if (!initializeFullWaveform(minHitDist_m,
                              maxHitDist_m,
                              minHitTime_ns,
                              maxHitTime_ns,
                              nsPerBin,
  ...
  populateFullWaveform(reflections,
                       fullwave,
                       distanceThreshold,
                       minHitTime_ns,
                       nsPerBin,
  ...
  int numReturns;
  digestFullWaveform(pointsMeasurement,
                     numReturns,
                     apMatrix,
                     fullwave,
                     intersects,
...
FullWaveformPulseRunnable::initializeFullWaveform(double const minHitDist_m,
                                                  ...
                                                  int& numFullwaveBins)
{
  return scanner->initializeFullWaveform(minHitDist_m,
                                         maxHitDist_m,
                                         minHitTime_ns,
                                         maxHitTime_ns,
                                         nsPerBin,
  ...
  int const peakIntensityIndex)
{
  ...
  vector<double> const& time_wave =
    scanner->getTimeWave(pulse.getDeviceIndex());
  ...
  for (it = reflections.begin(); it != reflections.end(); ++it) {
    ...
    double const wavePeakTime_ns =
      entryDistance_m / SPEEDofLIGHT_mPerNanosec; // in nanoseconds
    int const binStart =
...
)
{
  ...
  for (int i = 0; i < numFullwaveBins; ++i) {
    ...
    if (closestIntersection != nullptr && closestIntersection->prim.isValid()) {
      if (closestIntersection->prim.part != nullptr) {
      ...
      std::shared_ptr<Material> mat =
        PrimitiveAccessor::getMaterial(closestIntersection->prim);
      if (mat != nullptr)
  ...
  for (size_t i = 0; i < numRecords; ++i) {
    if (capturedIndices.find(i) == capturedIndices.end()) {
      ...
      calcIntensityRecords.erase(calcIntensityRecords.begin() + i -
                                 nonCapturedCount);
      calcIntensityIndices.erase(calcIntensityIndices.begin() + i -
                                 nonCapturedCount);
      ++nonCapturedCount;
    }
  }
...
bool
FullWaveformPulseRunnable::handleFullWaveformBin(
  std::vector<double> const& fullwave,
  MarquardtFitter& fit,
  double& echoWidth,
  int const binIndex,
  ...
  double const nsPerBin)
{
  ...
  if (!detectPeak(binIndex, winSize, fullwave, eps))
    return true;
...
FullWaveformPulseRunnable::findIntersection(vector<double> const& tMinMax,
                                            glm::dvec3 const& o,
                                           ...
                                           RandomnessGenerator<double>& rg2)
{
  ...
  if (fwDetector->fwfYielder != nullptr) {
    fwDetector->fwfYielder->push(FullWaveform(fullwave,
                                              fullwaveIndex,
                                              min_time,
                                              max_time,
                                              beamOrigin,
